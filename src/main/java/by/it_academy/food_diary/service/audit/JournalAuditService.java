...
public class JournalAuditService {
    ...
    public JournalAuditService(IAuditService auditService, IUserService userService, UserHolder userHolder) {
        this.auditService = auditService;
        this.userService = userService;
        this.userHolder = userHolder;
    }

    @AfterReturning("execution(* by.it_academy.food_diary.service.JournalService.save(..))")
    public void save(JoinPoint joinPoint) {
     createAudit(joinPoint,"Create");
    }

    @AfterReturning("execution(* by.it_academy.food_diary.service.JournalService.update(..))")
    public void update(JoinPoint joinPoint) {
       createAudit(joinPoint,"Update");
    }

   @AfterReturning("execution(* by.it_academy.food_diary.service.JournalService.delete(..))")
    ...
    private void createAudit(JoinPoint joinPoint, String method) {
        try {
            ...
            JournalDto journalDto = (JournalDto) args[0];
            Audit audit = new Audit();
            audit.setCreationDate(LocalDateTime.now());
            audit.setDescription(method+" journal " + journalDto.getId());
            String login = userHolder.getAuthentication().getName();
            User user = userService.findByLogin(login);
            audit.setUser(user);
            audit.setEssenceName("Journal");
            audit.setEssenceId(journalDto.getId());
